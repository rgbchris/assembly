...
_start:
 ...
 pushq %eax     # save the first answer before
 ...
 popq %ebx      # the second answer is already
                # in %eax. We saved the first
                # answer onto the stack, so now
                # we can just pop it out into 
                # %ebx
...
end_power:
 movl -4(%ebp), %eax  # return values goes in %eax
 movl %ebp, %esp      # restore the stack pointer
 popq %ebp            # restore the base pointer
 ret
